...
class FuzzyMatcher:
    ...
    def _match_row(
        ...
        # Ensemble: max(WRatio, partial_ratio) handles both symmetric and
        # asymmetric-length name pairs (e.g. short branch vs long HQ name).
        if self._ensemble:
            scores_w = rfprocess.cdist(
                [left_name_norm], right_names_norm,
                scorer=fuzz.WRatio, processor=None,
                score_cutoff=self.cfg.score_threshold,
            )[0]
            scores_p = rfprocess.cdist(
                [left_name_norm], right_names_norm,
                scorer=fuzz.partial_ratio, processor=None,
                score_cutoff=self.cfg.score_threshold,
            )[0]
            import numpy as np
            scores = np.maximum(scores_w, scores_p)
        else:
